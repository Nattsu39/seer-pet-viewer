...
import { DEFAULT_BATTLE_PX_PER_UNIT } from "@seer-pet-anim/battle-layout";
import { MAX_EXPORT_SIDE } from "./export-dimensions.js";
import { planBattleViewportExport } from "./battle-viewport.js";

// ppu=100 → 125 px/单位；左锚点 x=960-4×100=560、右锚点 x=1360，锚点 y=540
...
describe("planBattleViewportExport", () => {
  it("左侧指定单位像素比：固定 1920×1080 设计帧，锚点像素位置恒定", () => {
    ...
    expect(layout.pixelsPerUnitX).toBeCloseTo(125);
    expect(layout.pixelsPerUnitY).toBeCloseTo(125);
  });
  ...
  it("maxSide 放宽后倍率生效：2× → 3840×2160，锚点与密度同倍放大", () => {
    ...
    expect(layout.pixelsPerUnitY).toBeCloseTo(250);
    expect(layout.originX).toBeCloseTo(1120);
